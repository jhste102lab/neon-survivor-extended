...
설치도 다운로드도 계정도 필요 없습니다. PC나 모바일에서 페이지를 열고 바로 시작하세요. 현재 v1.2.1 Cloudflare 빌드는 플레이어 피드백을 참고한 **차원균열/런 설정 오버홀**을 포함합니다. 이벤트와 차원균열 분리, 대기 없는 랜덤 차원 시련, 차원 유물/보상 카드, 후반 자석 강화, 실제 수치 도감, 오버클럭 7% 표기 수정이 추가·개선되었습니다.

## 📝 v1.2.1 패치노트 — 차원 모드 오버홀

플레이어 피드백의 핵심인 “10분 이후 반복감”을 줄이기 위해, 후반 루프 앞에 직접 선택하고 클리어하는 차원 모드를 추가했습니다.

- 10분 이후 차원균열이 발생하면 대기 없이 랜덤 차원 시련으로 떨어지고, 미션을 클리어하면 원래 전장으로 복귀합니다.
- 일반 이벤트는 기존처럼 필드 위 목표와 소형 보상을 제공합니다. 차원균열은 별도 후반 시련으로 취급됩니다.
...
- 도감에 차원/실제 수치 표시를 추가하고, 오버클럭 설명을 실제 `무기 공격 간격 -7%` 효과에 맞췄습니다.
...
- **무기 진화 24종**: 모든 무기는 무기 만렙, 레시피 패시브 숙련, 진화 코어 조건을 만족하면 진화할 수 있습니다. 추가된 16종은 신규 회복/보호막 내구도 효과 없이 피해, 군중 제어, 조준, 이동 압박을 강화합니다.
- **차원균열 후반 전환**: 10분 이후 차원균열이 발생하면 랜덤 차원 시련으로 자동 진입하고, 미션을 클리어하면 원래 전장으로 복귀합니다.
