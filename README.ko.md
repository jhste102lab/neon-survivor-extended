...
설치도 다운로드도 계정도 필요 없습니다. PC나 모바일에서 페이지를 열고 바로 시작하세요. 현재 v1.1.10 Cloudflare 빌드는 상세 기록을 열어볼 수 있는 글로벌 리더보드, 24종 무기 진화, 10/13/16분 엔드리스 합체 보스 루프, 강화된 후반 보스 압박, 10분 이후 경험치 자동 흡수 가독성 보정, 장판 필드 보상의 강화 카드화, 카드 선택 시 수치 변화 표시, 일시정지 화면의 무기별 이펙트 표시 ON/OFF, 한 번이라도 켜면 리더보드를 막는 필드 테스트 무적 모드, 부드러운 밀집 몹 처리, 중첩 드롭, 한글화된 탑 피해/마지막 피격 기록을 포함합니다.
...
- **엔드리스 보스 자원 쟁탈**: 합체/엔드리스 보스가 포식을 예고하고 픽업을 쟁탈 상태로 만들며, 닭과 경험치 보석으로 회복하고, 폭탄 포식 시 회복량 기반 역류 피해를 받으며, 자석 포식과 다중 무기 봉인으로 후반 압박을 만듭니다. 봉인된 무기는 슬롯 잠금과 남은 시간으로 표시됩니다.
