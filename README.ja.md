...
インストールもダウンロードもアカウントも不要です。PC でもモバイルでも、ページを開けばすぐに遊べます。現在の v1.1.5 Cloudflare ビルドには、グローバルランキング、24 種類の武器進化、10/13/16 分へ続くエンドレス合体ボスループ、吸収・ドロップ封印・武器沈黙・操作歪曲のボス能力、8 分後に解放される 0.5x 精密速度、スタック式ドロップ、無敵表示、後半の自動視認性補正、モバイルでの敵 pop-in 軽減、密集戦のモバイル描画改善、クラッシュ後の再開復元が含まれます。
