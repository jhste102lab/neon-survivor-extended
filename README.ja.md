...
インストールもダウンロードもアカウントも不要です。PC でもモバイルでも、ページを開けばすぐに遊べます。現在の v1.1.10 Cloudflare ビルドには、詳細を開けるグローバルランキング、24 種類の武器進化、10/13/16 分へ続くエンドレス合体ボスループ、強化された後半ボス圧、10 分以降の経験値自動吸収による視認性改善、フィールドイベント報酬のアップグレードカード化、カード選択時の数値変化表示、一時停止画面での武器別エフェクト表示 ON/OFF、1 回でも有効化するとランキング送信を止めるフィールドテスト無敵モード、滑らかな密集敵処理、スタック式ドロップ、結果画面のダメージ/最後の被弾ラベル改善が含まれます。
