...
インストールもダウンロードもアカウントも不要です。PC でもモバイルでも、ページを開けばすぐに遊べます。現在の v1.2.4 Cloudflare ビルドには**ディメンションモード刷新**が含まれます。10 分のリフトゲートキーパー、5 秒滞在で入るポータル、自由に往復できるハブ、選択式の 8 ディメンション、崩壊時の自動脱出、ディメンション遺物/報酬カード、オープンソースのポータル装飾アセット、図鑑の実数値表示、Overclock の実効果「攻撃間隔 7% 短縮」に合わせた説明修正を追加しました。
