...
无需安装、无需下载、无需账号。打开网页即可在 PC 或移动端开局。当前 v1.2.0 Cloudflare 构建包含可查看详细记录的全球排行榜、24 种武器进化、10/13/16 分钟循环推进的无尽合体 Boss、更强的后期 Boss 压力、10 分钟后自动吸收经验以提升清晰度、场地事件奖励改为升级卡、选卡时显示数值变化、暂停界面按武器切换特效显示、开启过即禁止排行榜提交的场地测试无敌模式、顺滑密集怪群处理、叠加掉落，以及更清晰的伤害/最后命中结果标签。
