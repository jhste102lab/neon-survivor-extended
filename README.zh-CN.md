...
无需安装、无需下载、无需账号。打开网页即可在 PC 或移动端开局。当前 v1.2.1 Cloudflare 构建包含**维度模式重做**：10 分钟裂隙守门人、需要停留 5 秒的传送门、可自由往返的维度枢纽、8 个可选择并通关的维度、崩塌自动脱出、维度遗物/奖励卡、开源传送门装饰资源、图鉴实际数值显示，以及 Overclock 实际 7% 攻击间隔缩短的文本修正。
...
- **维度模式后期转场**：10 分钟出现裂隙守门人，击败后开启维度枢纽；玩家可选择 8 个维度通关，也可回到外部战场继续发育后再挑战。
- **无尽后期循环**：维度关门之后仍没有固定胜利画面，难度和 Boss 压力会继续提升。
- **霓虹伙伴**：6 种伙伴角色可加入，并可在后期获得后排共鸣。
- **场地事件与维度奖励**：裂隙、风暴、契约和补给事件提供限时风险/奖励目标；维度通关会提供固定遗物和奖励卡选择。
