...
无需安装、无需下载、无需账号。打开网页即可在 PC 或移动端开局。当前 v1.1.4 Cloudflare 构建包含全球排行榜、24 种武器进化、每 10 分钟重复出现的合体 Boss、8 分钟后解锁的 0.5x 精准速度、叠加掉落、无敌提示，以及后期自适应清晰度优化、移动端运行稳定性和崩溃后的继续恢复。
